...
.hero{background:linear-gradient(135deg,#0d1424 0%,#111827 60%,#130d24 100%);
    border:1px solid var(--border);border-radius:16px;padding:2rem 2.5rem;
    margin-bottom:1.5rem;position:relative;overflow:hidden;}
.hero::before{content:'';position:absolute;top:-60px;right:-40px;width:300px;height:300px;
    background:radial-gradient(circle,rgba(59,130,246,0.12),transparent 70%);pointer-events:none;}
.hero::after{content:'';position:absolute;bottom:-80px;left:30%;width:400px;height:200px;
    background:radial-gradient(circle,rgba(139,92,246,0.08),transparent 70%);pointer-events:none;}
.hero-title{font-size:2.2rem;font-weight:700;letter-spacing:-0.03em;
    background:linear-gradient(90deg,#e2e8f0,#93c5fd 55%,#c4b5fd);
    -webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin:0 0 0.3rem;}
.hero-sub{font-size:0.95rem;color:var(--muted);margin:0 0 1rem;}
.badge{display:inline-block;background:rgba(59,130,246,0.1);border:1px solid rgba(59,130,246,0.25);
    border-radius:50px;padding:0.18rem 0.7rem;font-size:0.7rem;
    font-family:'JetBrains Mono',monospace;color:#93c5fd;margin-right:0.35rem;letter-spacing:0.04em;}
...
.kpi-label{font-size:0.65rem;color:var(--muted);text-transform:uppercase;
    letter-spacing:0.1em;margin-bottom:0.3rem;font-family:'JetBrains Mono',monospace;}
.kpi-val{font-size:1.6rem;font-weight:700;line-height:1;}
.kpi-sub{font-size:0.72rem;color:var(--muted);margin-top:0.2rem;}
.kpi-good{color:#10b981;}.kpi-warn{color:#f59e0b;}.kpi-bad{color:#ef4444;}.kpi-neutral{color:#e2e8f0;}

.section-hd{display:flex;align-items:center;gap:0.5rem;margin:1.5rem 0 0.8rem;}
.section-icon{width:26px;height:26px;background:rgba(59,130,246,0.12);
...
.insight-card{background:var(--surface);border:1px solid var(--border);
    ...
    padding:1.1rem 1.3rem;margin-bottom:0.75rem;font-size:0.88rem;line-height:1.7;}
.insight-card h3{font-size:0.9rem;color:#93c5fd;margin:0 0 0.4rem;font-weight:600;}
.insight-card p{margin:0;color:var(--text);}

.finding{background:var(--surface2);border:1px solid var(--border);border-radius:8px;
    padding:0.7rem 1rem;margin-bottom:0.5rem;font-size:0.83rem;
    display:flex;align-items:flex-start;gap:0.6rem;}
.finding-icon{font-size:1rem;flex-shrink:0;margin-top:0.05rem;}
.finding-text{color:var(--text);line-height:1.5;}
...
.stTabs [data-baseweb="tab"]{border-radius:7px!important;color:var(--muted)!important;
    font-size:0.82rem!important;font-family:'Space Grotesk',sans-serif!important;
    font-weight:500!important;padding:0.38rem 0.9rem!important;}
...
.stButton>button{background:linear-gradient(135deg,var(--accent),var(--accent2))!important;
    ...
    font-family:'Space Grotesk',sans-serif!important;font-weight:600!important;font-size:0.85rem!important;}
.stRadio>div{gap:0.5rem!important;}
[data-testid="stSidebar"]{background:var(--surface)!important;border-right:1px solid var(--border)!important;}
.stDataFrame{border-radius:10px;overflow:hidden;}
.stSuccess{background:rgba(16,185,129,0.08)!important;border:1px solid #10b981!important;border-radius:8px!important;}
.stWarning{background:rgba(245,158,11,0.08)!important;border:1px solid #f59e0b!important;border-radius:8px!important;}
.stError{background:rgba(239,68,68,0.08)!important;border:1px solid #ef4444!important;border-radius:8px!important;}
</style>
...
def sec(icon, title):
    st.markdown(f'<div class="section-hd"><div class="section-icon">{icon}</div>'
                f'<p class="section-title">{title}</p></div>', unsafe_allow_html=True)

def finding(icon, text):
    st.markdown(f'<div class="finding"><span class="finding-icon">{icon}</span>'
                f'<span class="finding-text">{text}</span></div>', unsafe_allow_html=True)
...
<div class="hero">
  ...
  <p class="hero-sub">Upload any CSV — get instant business insights, statistical analysis, and AI-powered recommendations.</p>
  <span class="badge">Business EDA</span><span class="badge">Statistical Analysis</span>
  <span class="badge">AI Insights</span><span class="badge">Interview-Ready</span>
</div>
""", unsafe_allow_html=True)
...
if uploaded is None:
    ...
    with c2:
        st.markdown("""
        <div style="border:2px dashed #1e2d45;border-radius:14px;padding:3.5rem 2rem;
             text-align:center;background:#111827;margin-top:1rem;">
          <div style="font-size:3rem;margin-bottom:1rem">📊</div>
          <p style="color:#e2e8f0;font-weight:600;font-size:1.1rem;margin:0 0 0.5rem">Drop your CSV here</p>
          <p style="color:#64748b;font-size:0.88rem;margin:0">Sales data · Customer data · Financial reports · Any tabular CSV</p>
...
st.markdown(f"""
<div class="kpi-grid">
  <div class="kpi"><div class="kpi-label">Rows</div>
    <div class="kpi-val kpi-neutral">{df.shape[0]:,}</div>
    <div class="kpi-sub">observations</div></div>
  <div class="kpi"><div class="kpi-label">Columns</div>
    <div class="kpi-val kpi-neutral">{df.shape[1]}</div>
    <div class="kpi-sub">{len(num_cols)} numeric · {len(cat_cols)} categorical</div></div>
  <div class="kpi"><div class="kpi-label">Missing</div>
    <div class="kpi-val {miss_cls}">{miss_pct}%</div>
    <div class="kpi-sub">{missing:,} cells affected</div></div>
  <div class="kpi"><div class="kpi-label">Duplicates</div>
    <div class="kpi-val {dupe_cls}">{dupes:,}</div>
    <div class="kpi-sub">{"clean ✓" if dupes==0 else "rows to review"}</div></div>
  <div class="kpi"><div class="kpi-label">Memory</div>
    <div class="kpi-val kpi-neutral">{round(df.memory_usage(deep=True).sum()/1024,1)}<span style="font-size:1rem"> KB</span></div>
    <div class="kpi-sub">in-memory</div></div>
</div>""", unsafe_allow_html=True)
...
with tabs[5]:
    ...
    # Volume
    findings.append(("📦", f"Dataset contains <b>{df.shape[0]:,} records</b> across <b>{df.shape[1]} fields</b>."))

    # Data quality
    if miss_pct == 0 and dupes == 0:
        findings.append(("✅", "Data quality is <b>excellent</b> — no missing values or duplicates detected."))
    else:
        if miss_pct > 0:
            findings.append(("⚠️", f"<b>{miss_pct}% missing data</b> detected. Imputation or removal required before modeling."))
        if dupes > 0:
            findings.append(("⚠️", f"<b>{dupes:,} duplicate rows</b> ({dupes/len(df)*100:.1f}%) may skew aggregations and model training."))
    ...
    if num_cols:
        for col in num_cols:
            ...
            if abs(s.skew()) > 1.5:
                ...
                findings.append(("📊", f"<b>{col}</b> is heavily skewed {direction} (skew={s.skew():.2f}). Consider log transformation for modeling."))
            if cv > 80:
...
if len(num_cols) >= 2:
    ...
    for col in cat_cols:
        un = df[col].nunique()
        if un == len(df):
            findings.append(("🆔", f"<b>{col}</b> has all unique values — likely an ID column. Exclude from modeling."))
        elif un == 1:
            findings.append(("⚠️", f"<b>{col}</b> has only 1 unique value — zero variance. Drop this column."))
        elif un > 50:
            findings.append(("🏷️", f"<b>{col}</b> has high cardinality ({un} categories). Consider encoding or grouping before ML."))

    # Date insight
    if date_cols:
        for dc in date_cols:
            span = (df[dc].max() - df[dc].min()).days
            findings.append(("📅", f"<b>{dc}</b> spans <b>{span} days</b> ({df[dc].min().date()} → {df[dc].max().date()})."))
...
with tabs[6]:
    ...
    st.markdown('<p style="color:#64748b;font-size:0.88rem;margin-bottom:1rem">Powered by Claude · Ask anything about your dataset — business questions, modeling advice, anomaly explanations.</p>', unsafe_allow_html=True)
    ...
    if st.button("🔍 Analyze", use_container_width=False):
        if not question.strip():
            ...
            finding_txt= "\n".join([f"{i}. {t.replace('<b>','').replace('</b>','')}" for i,(_,t) in enumerate(findings,1)])
